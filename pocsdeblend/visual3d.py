...
def explode_volume(volume, t=None, x=None, y=None,
    ...
    fig.suptitle(title, fontsize=18, y=0.94) # fontweight='bold',
    if ratio is None:
        wr = (nx, ny)
        hr = (ny, nx)
    else:
...
def explode_volume2(volume, x=None, y=None,
                    figsize=(8, 8), cmap='bone', clipval=None, p=98,
                    tlim=None, xlim=None, ylim=None,
                    tcrop=None, xcrop=None, ycrop=None,
                    labels=('[s]', '[km]', '[km]'),
                    tlabel='t', xlabel='x', ylabel='y',
                    ratio=None, linespec=None, interp=None,
                    removeaxis=False, title='',
                    filename=None, save_opts=None):
    """Display IL and XL of 3D volume
    ...
    # right plot
    ax1.imshow(volume[:, x], extent=[ylim[0], ylim[1], tlim[1], tlim[0]], **opts)
    ax1.axvline(x=yline, **linespec)
    if ycrop is not None:
        ax1.set_xlim(ycrop)
    if tcrop is not None:
        ax1.set_ylim(tcrop[1], tcrop[0])
